...
impl Default for LLM {
    fn default() -> Self {
        let transformer_block = TransformerBlock::new(consts::EMBEDDING_DIM, consts::HIDDEN_DIM);
...
impl LLM {
    pub fn network_description(&self) -> String {
        ...
        for (i, layer) in self.network.iter().enumerate() {
            let layer_type = match layer {
                ...
                LayerType::OutputProjection(_) => "OutputProjection",
            };
            description.push_str(&format!("Layer {}: {},", i + 1, layer_type));
        }
        description
    }
    ...
    fn forward(&mut self, text: &str) -> Vec<usize> {
        ...
        if input_len >= consts::MAX_SEQ_LEN {
            return output_tokens;
        }

        for _ in 0..(consts::MAX_SEQ_LEN - input_len) {
            // let tokenized_clone = tokenized.clone();

            // Check if we're approaching the maximum sequence length
            if output_tokens.len() >= consts::MAX_SEQ_LEN - 1 {
                break;
            }
